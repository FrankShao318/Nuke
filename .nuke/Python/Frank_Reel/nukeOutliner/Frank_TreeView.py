...
class TreeView(QtWidgets.QTreeView):
    ...
    def __init__(self, color_setting=None):
        ...
        # If TreeView does not have any items, create new model.
        ...
        # Delegate.
        delegate = QtWidgets.QItemDelegate(parent=self)
    ...
    def add_headers(self, headers):
        ...
        Args:
            headers(list): The headers list of TreeView. [(header name, resize mode),]
        """
    ...
    def add_items(self, items_kwargs, unique_name=True, parent_item=None):
        ...
        model.setColumnCount(len(items_kwargs))
        ...
        for row_items_kwargs in items_kwargs:
            ...
            for column_item_kwargs in row_items_kwargs:
                # Set column item.
                column_item = self._create_item(items_kwargs=column_item_kwargs, unique_name=unique_name)
    ...
    def duplicate_items(self, leaves=True):
            ...
            for hierarchy in items:
                ...
                for row in hierarchy['children']:
                    ...
                    for column in row:
                        ...
                        # If leaves, duplicate column item itself, with duplicate ites leaves.
        ...
        # If not selected, avoid calling self.get_items to query all the items.
            ...
            nuke.message('No items in the TreeView are selected to be duplicated!')
    ...
    def select_items(self, names):
        ...
        for name in names:
            ...
            self.selectionMode().clear()
            self.selectionMode().select(selection, flags)
    ...
    def parent_items(self):
        ...
        if indexes:
            ...
            for hierarchy in items:
                parent_item = hierarchy['parent']['item']
                # Only process if selected items' parent is not in parent_list.
                if str(parent_item) not in parents_list:
                    # Rows level.
                    rows_nums = [
        ...
        # If not selected, avoid calling self.get_items to query all the items.
            ...
            nuke.message('No items in the TreeView are selected to get parented!')
    ...
    def unparent_items(self):
        ...
        Returns:
            dict/None: The unparent children hierarchy dictionary with
            {
            key ('parent'):
            value ({'item': QtGui.QStandardItem, 'position': (row, column), 'kwargs': parent_kwargs}),
            key ('children'):
        ...
        # If not selected, avoid calling self.get_items to query all the items.
            ...
            nuke.message('No items in the TreeView are selected to get unparented!')
    ...
    def group_items(self, group_columns_kwargs):
        ...
        # If not selected, avoid calling self.get_items to query all the items.
            ...
            nuke.message('No items in the TreeView are selected to group! An empty group created.')
    ...
    def ungroup_items(self):
        ...
        if indexes:
            ...
            for hierarchy in items:
                ...
                try:
                    # There are cases groups already be ungroup and deleted.
                    row_num = parent_item.rowCount()
                    ...
                    for row in hierarchy['children']:
                        ...
                        for i in reversed(range(row[0]['item'].rowCount())):
                            ...
                            parent_item.inserRow(row_num, child_columns_items)
                            children_items.append(child_columns_items)

                    # Only delete group if have children.
                    if children_items:
                        # Avoiding parent's row index changes, delete from back -> forward.
        ...
        # If not selected, avoid calling self.get_items to query all the items.
            ...
            nuke.message('No items in the TreeView are selected to ungroup!')
...
class ItemDelegate(QtWidgets.QStyledItemDelegate):
    ...
    def createEditor(self, parent, option, index):
        ...
        if isinstance(widget, QtWidgets.QComboBox):
            # Set enum.
            widget.addItem(item_kwargs['enum'])
    ...
    def setModelData(self, editor, model, index):
        ...
        # Set data, so when edit with invalid characters, it will show immediately the converted (str) value.
