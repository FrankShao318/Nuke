...
# Usage:
# 
# Create a NoOp node which allows the user to control motionblur on all nodes globally.
#----------------------------------------------------------------------------
...
def moblur_controller():
    ...
    # Put disable check box in a new line.
    ...
    # Define another function, so when NoOp node gets deleted, all expression-links are removed and node values are set back to default.
